...
export const EditFoodProduct = observer(() => {
  const {id} = useParams();
  const navigate = useNavigate();

  const formik = useFormik({
    initialValues: {
      product: foodsStore.isSingleFoodProduct?.product?._id || '',
      amount: foodsStore?.isSingleFoodProduct?.amount,
    },
    onSubmit: (values, {setSubmitting}) => {
      setSubmitting(true);
      const editProductData: IAddOneFoodProduct = {
        id: id!,
        product: values.product!,
        amount: values.amount!,
      };

      foodsApi.updateFoodProduct(editProductData)
        .then(() => {
  ...
  const handleClose = () => {
    foodsStore.setIsOneFoodProductEditModal(false);
  ...
  return (
    <Modal
      open={foodsStore.isOneFoodProductEditModal}
